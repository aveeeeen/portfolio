...
<template>
  <div class="center">
    <div @click="onChange()" @change="onChange()" class="synth-kit">
      ...
      <SnareSettings :osc-settings="synthState.oscSettings" @update="event => synthState.oscSettings = event">
      </SnareSettings>

      <div class="container">
        <h2>Decay / Sustain</h2>
        <div class="flex-vert">
          <DrumEnvelope :envelope="synthState.ampEnv" @update-envelope="event => synthState.ampEnv = event">
          </DrumEnvelope>
          <p>Contour</p>
          <input @change="mapContourToSynth(synthState.modEnv.decay, synthState)" class="pitch-slider" type="range"
            min="0" max="2" step="0.01" v-model.number="synthState.modEnv.decay">
        </div>
      </div>

      <div class="container">
        <h2>Sweep</h2>
        <DrumSweepEnvelope :sweep="synthState.pitchEnv" @update="event => synthState.pitchEnv = event">
        </DrumSweepEnvelope>
      </div>

      <div class="container">
        <h2>lpf</h2>
        <DrumFilterEnvelope :filter-env="synthState.lpfEnv" @update="event => synthState.lpfEnv = event">
        </DrumFilterEnvelope>
      </div>

      <div class="container">
        <h2>hpf</h2>
        <DrumFilterEnvelope :filter-env="synthState.hpfEnv" @update="event => synthState.hpfEnv = event">
        </DrumFilterEnvelope>
      </div>
    </div>
  </div>
