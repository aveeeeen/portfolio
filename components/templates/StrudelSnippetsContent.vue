...
const data = useFetch<StrudelSnippetsBlocksResult>(() => (snippetId.value ? `/api/strudel-snippets/${snippetId.value}` : ''));
const snippet = computed(() => data.data.value ?? undefined);
...
const parsedHtml = computed(() => {
  if (!snippet.value || !snippet.value.blocks) {
    return '';
  }
  return parse(snippet.value.blocks);
});
...
const tocLinks = computed(() => {
  ...
  return getTableOfContents(snippet.value.blocks);
});
